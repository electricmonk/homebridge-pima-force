...
interface DriverFixture {
  driver: PimaDriver;
  /** Port the production driver's TCP server is listening on. */
  port: number;
}

interface DriverFixtureOpts {
...
async function setupDriver(opts: DriverFixtureOpts = {}): Promise<DriverFixture> {
...
async function connectAlarm(
  ...
  const alarm = anAlarmSystem({ port: fix.port, account: opts.account ?? 1234 });
  const driverConnected = once(fix.driver, 'connected');
  await alarm.connect();
...
describe('PimaDriver — connection lifecycle', () => {
  let fix: DriverFixture;
  let alarm: AlarmSystem | null = null;
  beforeEach(async () => { fix = await setupDriver(); });
  afterEach(async () => {
    alarm?.close();
    alarm = null;
    await fix.driver.stop();
  });

  it('emits connected when the alarm dials in', async () => {
    alarm = await connectAlarm(fix, { verify: false });
    assert.equal(fix.driver.isConnected(), true);
  });

  it('emits verified after first frame with matching account', async () => {
    alarm = await connectAlarm(fix, { verify: false });
    const verified = once(fix.driver, 'verified');
    await alarm.verify();
    await verified;
  });
  ...
  it('arm() rejects before panel is verified', async () => {
    alarm = await connectAlarm(fix, { verify: false });
    await assert.rejects(fix.driver.arm(1), /not yet verified/);
  });

  it('emits disconnected when the alarm drops', async () => {
    alarm = await connectAlarm(fix, { verify: false });
    const off = once(fix.driver, 'disconnected');
    alarm.close();
    await off;
    assert.equal(fix.driver.isConnected(), false);
  });
});
...
describe('PimaDriver — receive side', () => {
  ...
  it('emits zone active=true when zone opens', async () => {
    alarm = await connectAlarm(fix);
    const off = once(fix.driver, 'zone');
  ...
  it('emits disarm with source=remote on remote disarm event', async () => {
    alarm = await connectAlarm(fix);
    const off = once(fix.driver, 'disarm');
  ...
  it('emits unknown for unrecognized event types', async () => {
    alarm = await connectAlarm(fix);
    const off = once(fix.driver, 'unknown');
  ...
  it('emits system commPath ok=true on comm-path restore', async () => {
    alarm = await connectAlarm(fix);
    const off = once(fix.driver, 'system');
...
describe('PimaDriver — send side (arm/disarm)', () => {
  ...
  it('arm() rejects for an unknown partition', async () => {
    await assert.rejects(fix.driver.arm(99), /partition 99 not configured/);
  });

  it('getZoneNames() sends a DATA-REQ for the zone-names range', async () => {
    const pending = fix.driver.getZoneNames(1, 16);
  ...
  it('getZoneCount() sends a DATA-REQ for the installed zone count', async () => {
    const pending = fix.driver.getZoneCount();
  ...
  it('getSystemKeyStatus() queries the partition\'s status using its own user code', async () => {
    const pending = fix.driver.getSystemKeyStatus(2);
  ...
  it('getSystemKeyStatus() rejects for an unconfigured partition', async () => {
    await assert.rejects(fix.driver.getSystemKeyStatus(99), /partition 99 not configured/);
  });

  it('requestData uses params.password when both that and a default user code are available', async () => {
    const pending = fix.driver.requestData({ id: 260, startOrder: 1, stopOrder: 16, password: '9999' });
  ...
  it('requestData resolves with the DATA frame parameters', async () => {
    const pending = fix.driver.requestData({ id: 260, startOrder: 1, stopOrder: 3 });
    const q = await alarm!.nextQuery({ id: 260 });
  ...
  it('requestData propagates more:"yes" to the caller', async () => {
    const pending = fix.driver.requestData({ id: 260, startOrder: 1, stopOrder: 16 });
    const q = await alarm!.nextQuery({ id: 260 });
  ...
  it('serializes concurrent requestData calls (one DATA-REQ on the wire at a time)', async () => {
    const a = fix.driver.requestData({ id: 260, startOrder: 1, stopOrder: 1 });
    const b = fix.driver.requestData({ id: 260, startOrder: 2, stopOrder: 2 });
  ...
  it('requestData rejects with a NAK that matches its counter', async () => {
    const pending = fix.driver.requestData({ id: 260, startOrder: 1, stopOrder: 1 });
    const q = await alarm!.nextQuery({ id: 260 });
...
describe('PimaDriver — OPERATION request/response semantics', () => {
  ...
  beforeEach(async () => {
    fix = await setupDriver();
    alarm = await connectAlarm(fix);
    alarm.autoAck.operations = false; // tests model ACK / NAK / no-response themselves
  });
  afterEach(async () => {
    alarm?.close();
    alarm = null;
  ...
  it('arm() does not resolve until the panel ACKs', async () => {
    const armPromise = fix.driver.arm(2);
  ...
  it('serializes concurrent OPERATIONs (one on the wire at a time)', async () => {
    const armP = fix.driver.arm(1);
    const disarmP = fix.driver.disarm(1);
  ...
  it('shares the queue between OPERATION and DATA-REQ — DATA-REQ waits for the prior OPERATION to settle', async () => {
    const armP = fix.driver.arm(1);
    const reqP = fix.driver.getZoneCount();
...
describe('PimaDriver — request timeout', () => {
  ...
  beforeEach(async () => {
    fix = await setupDriver({ requestTimeoutMs: 80 });
    alarm = await connectAlarm(fix);
  ...
  it('rejects arm() when the panel does not respond within the timeout', async () => {
    await assert.rejects(fix.driver.arm(2), /timeout waiting for response to operation/);
  });

  it('rejects requestData() when the panel does not respond within the timeout', async () => {
    await assert.rejects(fix.driver.getZoneCount(), /timeout waiting for response to data-req/);
  });

  it('keeps the queue alive after a timeout — the next call still works', async () => {
    await assert.rejects(fix.driver.arm(2), /timeout/);
...
describe('PimaDriver — inbound retransmit dedup', () => {
  ...
  it('emits a typed event only once for back-to-back retransmits, but ACKs each one', async () => {
    ...
    fix.driver.on('zone', (e) => zoneEvents.push(e));
...
describe('PimaDriver — requestData password override', () => {
  it('requestData succeeds with params.password when no partitions configured', async () => {
    const fix = await setupDriver({ partitions: [], account: 5678, opCounterStart: 1 });
    const alarm = await connectAlarm(fix, { account: 5678 });
    try {
      const pending = fix.driver.requestData({ id: 260, startOrder: 1, password: 'override' });
      const q = await alarm.nextQuery({ id: 260 });
  ...
  it('requestData rejects when no partitions and no password', async () => {
    const fix = await setupDriver({ partitions: [], account: 5678 });
    try {
      await assert.rejects(
        fix.driver.requestData({ id: 260, startOrder: 1 }),
...
describe('PimaDriver — send without connection', () => {
  it('arm() rejects when no panel is connected', async () => {
    const fix = await setupDriver({ partitions: [{ id: 1, userCode: '1111' }] });
    try {
      await assert.rejects(fix.driver.arm(1), /no active panel connection/);
    } finally {
      await fix.driver.stop();
    }
  });
...
describe('PimaDriver — reverseStrings option', () => {
  it('reverses every string parameter in DATA responses when enabled', async () => {
    // Build the driver with reverseStrings: setupDriver doesn't expose the
    // flag (it's deprecated), so construct directly.
    const driver = new PimaDriver({
      port: 0,
      account: 1234,
      partitions: [{ id: 1, userCode: '1111' }],
    ...
    await driver.start();
    const addr = driver.address() as import('node:net').AddressInfo;
    const fix: DriverFixture = { driver, port: addr.port };
    const alarm = await connectAlarm(fix);
    try {
      const pending = fix.driver.requestData({ id: 260, startOrder: 1, stopOrder: 2 });
      const q = await alarm.nextQuery({ id: 260 });
